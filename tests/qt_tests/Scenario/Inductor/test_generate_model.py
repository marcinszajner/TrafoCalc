...
def test_normal_model(TrafoCalc):
    ...
    path = 'tests\\qt_tests\\Scenario\\Inductor\\' + name + '\\'
    QtCore.QTimer.singleShot(50, lambda: keyboard.write(path + name + '.json'))
    QtCore.QTimer.singleShot(200, lambda: keyboard.send('enter'))
    QtCore.QTimer.singleShot(1000, lambda: keyboard.write(path + name + '.FEM'))
    QtCore.QTimer.singleShot(1250, lambda: keyboard.send('enter'))
    QtCore.QTimer.singleShot(1500, lambda: keyboard.send('tab'))
    QtCore.QTimer.singleShot(1750, lambda: keyboard.send('enter'))
